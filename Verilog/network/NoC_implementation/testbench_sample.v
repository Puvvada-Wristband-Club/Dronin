    ...
    begin
      ...
      for(i = 0; i < `NUM_USER_SEND_PORTS; i = i + 1)
        begin
          ...
          if(send_flit[i]) begin

            for(j = 0; j < `NUM_VCS; j = j + 1) 

              if(counter[j] != 0'b1) begin
                //send_credit[i] <= 1'b1;

                counter[flit_in[i][flit_port_width-8:7]] <= 
                counter[flit_in[i][flit_port_width - 8:7]] - 1;

                send_flit[i] = 0'b1; 

                $display("virtual channel %d is currently %x: ", 
                flit_in[i][flit_port_width-8:7], counter[flit_in[i][flit_port_width-8:7]]);
              end
              
              //else begin
                //$display("virtual channel %d is full!!", flit_in[i][flit_port_width-8:7]);
             // end
            //end
          end
